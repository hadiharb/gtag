...
static float rowW[19]={1.0f,1.0f,1.0f,1.0f,1.0f,1.0f,1.0f,1.0f,1.0f,1.0f,
			1.0f,1.0f,1.0f,1.0f,1.0f,1.0f,1.0f,1.0f,1.0f};



static void fp_distance1 (int max1, int max2, char* query, char * data,int stepData, float & distOut,int &indOut,int &indOut1,int &indOut2)
{
    int *hit;
...
static void fp_distanceStatic (int offset,int max, char* query, char * data, float & distOut)
{
    int hit=0;
...
int fp_compare(CFingerPrint *s1, CFingerPrint *s2, int &startat, bool HD)

{
    ...
    float bufD[30],dista=0;
    int bufI[30];
    int where=0;
    int from = (HD)?1:CF_SLOW_COMPARE_L;
    int to = (HD)?10:CF_SLOW_COMPARE_H;

    for (int i=from; i<to; i+=1)
    {
        float dist;
        fp_distance1 (s1->N,s2->N,s1->fingerprint[i],s2->fingerprint[i],1,dist,indO,indO1,indO2);
        bufI[where]=indO;
        where++;
        bufI[where]=indO1;
        where++;
    ...
    for (int R=0; R<lastwhere; R++)
    {
        ...
        {
            float dist;
            fp_distanceStatic (bufI[R],s1->N,s1->fingerprint[i],s2->fingerprint[i],dist);
//	    printf("%i/%i->%f\n",R,i,dist);
            dista+=dist*rowW[i] ;
        }
        bufD[where]=dista;
...
CFingerPrint::CFingerPrint(int n, char *fp[19])
{
    N=0;
    for (int i=0; i<19; i++)
    {
...
void CFingerPrint::fill (int n, char *fp[19])
{
    for (int i=0; i<19; i++)
    {
	if (fingerprint[i]) {
    ...
    for (int i=0; i<19; i++)
    {
        fingerprint[i] = new char[N+1];
        memcpy(fingerprint[i],fp[i],n);
    }
}

void CFingerPrint::fill (int n, char *fp[19], int step)
{
    for (int i=0; i<19; i++)
    {
	if (fingerprint[i]) {
...
bool CFingerPrint::save(const char *fname)
{
    ...
    for (int s=0; s<19; s++)
    {
        fwrite (this->fingerprint[s],1,N,f);
        fwrite (&ch, 1,1, f);
    }
    fclose (f);
    return true;
...
int CFingerPrint::open(const char *fname)
{
    struct flock fl;
    ...
    for (int i=0; i<19; i++)
    {
        if (fingerprint[i]) delete fingerprint[i];
        fingerprint[i] = new char[N+1];
        memcpy(fingerprint[i],buf+4+i*(N+1),N);
    }
    delete[] buf;
    return CFP_OK;
}
...
int CFingerPrint::compare_to(CFingerPrint* dest, int &pos, bool full)
{
    if (!dest || N==0) return -1;
    if (dest->N==0) return -1;
    int r = fp_compare(this, dest, pos, full);
    pos = pos*100;
    return r;
}
...
int CFingerPrint::compare_to_slow(CFingerPrint* dest, int &pos, bool full)
{
    if (!dest || N==0) return -1;
    if (dest->N==0) return -1;
    int r = fp_compare(this, dest, pos, full);
